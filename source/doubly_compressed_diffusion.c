        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    double *A, *C, *w_0, *u, *d, *mu, *w_star, *e, *w_k ,*H, *H_grad;
    
    /* check for proper number of arguments */
    if(nrhs!=11) {
        mexErrMsgIdAndTxt("DiffusionToolbox:doubly_compressed_diffusion:nrhs","Eleven inputs required.");
    }
    if(nlhs!=2) {
    ...
    if (mxGetScalar(prhs[8])>L){
        mexErrMsgIdAndTxt("DiffusionToolbox:doubly_compressed_diffusion:notScalar", "M must less then or equal to L.");
    }
    ...
    doubly_compressed_diffusion(N, L, K, M, M_grad, w_star, w_0, A, C, mu, d, u, H, H_grad, e, w_k);
