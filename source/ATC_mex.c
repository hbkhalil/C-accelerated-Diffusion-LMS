        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if(mxGetM(prhs[2])!=L) {
        mexErrMsgIdAndTxt("DiffusionToolbox:ATC_mex:notDouble"," w_0 must as much lines data model .");
    }
    ...
    if(mxGetM(prhs[6])!=L) {
        mexErrMsgIdAndTxt("DiffusionToolbox:ATC_mex:notDouble","w_star length must be the same as w_0 ");
    }
...
//     arrayProduct(multiplier,inMatrix,outMatrix,(mwSize)ncols);
    ...
    ATC(N, L, K, w_star, w_0, A, C, mu, d, u, e, w_k);
